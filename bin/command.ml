...
module Register = struct
  ...
  let pointers: string list = (objective_add "pc" >> unary_set (register "pc") 0) :: [objective_add "sp" >> unary_set (register "sp") 0]
...
module Stack = struct
  ...
  let access size reg index_in_reg = 
    write_to_file (file_name "access" reg) (generate_access size index_in_reg reg |> flatten);
